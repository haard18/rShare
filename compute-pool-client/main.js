...
ipcMain.handle("create-pool", async (event, poolData) => {
  ...
  console.log(
    `🚀 Compute Pool Created | Name: ${name}, Port: ${port}, Owner: ${poolOwnerId}, Type: ${poolTemplate}, Task: ${taskTemplate}, Approach: ${approach}`
  );
  ...
  wss.on("connection", (ws) => {
    ...
    ws.on("close", () => {
      ...
      if (agentId && agents.has(agentId)) {
        ...
        console.log(
          `🚪 Agent ${agentId} disconnected. Updated Pool Resources:`,
          totalResources
        );
      }
    });
  ...
  return `✅ Compute Pool '${name}' created successfully on port ${port}`;
});
...
ipcMain.handle("get-pool-config", () => {
  return poolConfig ? poolConfig : "❌ No active pool";
});
